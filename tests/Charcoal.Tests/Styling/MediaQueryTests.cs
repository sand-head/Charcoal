...
namespace Charcoal.Tests.Styling;
...
public class MediaQueryTests
{
    ...
    public void Container_queries_take_a_name_and_the_container_box_feature_names()
    {
        ...
        Assert.True(query.Addresses(new Charcoal.Layout.Style { ContainerNames = ["main", "side"] }));
        Assert.False(query.Addresses(new Charcoal.Layout.Style { ContainerNames = ["main"] }));

        var anonymous = ContainerQuery.Parse("(width < 40)");
        Assert.Null(anonymous.Name);
        Assert.True(anonymous.Addresses(new Charcoal.Layout.Style()));
